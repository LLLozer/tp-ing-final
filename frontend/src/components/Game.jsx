...
export const Game = () => {
  ...
  const [selected, setSelected] = useState(null); // opción clickeada
  ...
  const handleAnswer = (optionIndex) => { //se le pasa la respuesta seleccionada
    if (selected !== null) return; //evita múltiples clics mientras muestra color
 ...
 const questions = [ 
  ...
  if (finished) { //renderiza esto si el juego termino
    return (
      <div className="min-h-screen bg-gradient-to-br flex flex-col items-center justify-center text-center text-black">
        <h1 className="text-4xl font-bold mb-6 drop-shadow-xl"> ¡Juego Terminado! </h1>
        ...
        <button
          ...
          className="px-6 py-3 bg-blue-600 hover:bg-blue-500 text-black font-semibold rounded-lg
                     shadow-[0_0_15px_#3b82f6] border border-blue-300 transition-all"
        >
          Volver a jugar
        </button>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br text-black px-4">

      <h2 className="text-center text-2xl font-bold mt-10 text-blue-300 drop-shadow-lg">
        ¡Pon a prueba tus conocimientos en Ingienería de de Software! 
      </h2>

      {!gameInit ? (
        <button 
          onClick={play} 
          className="block mx-auto mt-8 px-8 py-4 bg-blue-700 hover:bg-blue-600 text-black
                     rounded-lg shadow-[0_0_15px_#2563eb]
                     border border-blue-400 transition-all font-semibold"
        >
          JUGAR
        </button> 
      ) : (
        <div className="max-w-2xl mx-auto mt-12 p-8 rounded-xl bg-white/10 backdrop-blur-lg 
                        border border-blue-300/30 shadow-[0_0_20px_#1e3a8a]">

          {/* PROGRESO */}
          <p className="text-sm font-medium mb-2 text-black">
            Pregunta {index + 1} de {questions.length}
          </p>
          <div className="w-full bg-blue-950 h-3 rounded-full border border-blue-700 mb-5">
            <div
          ...
          <h2 className="text-lg font-semibold mb-6 text-black drop-shadow">
            {current.question}
          </h2>
          ...
          {current.options.map((opt, i) => {
            ...
            let base =
              "w-full text-left px-4 py-3 mb-3 rounded-lg border transition-all shadow-md font-medium";

            let color =
              "bg-blue-900/50 border-blue-500 text-black hover:bg-blue-700/50";

            if (selected !== null) {
              if (i === correct) color = "bg-green-600 text-black border-green-300";
              else if (i === selected) color = "bg-red-600 text-black border-red-300";
              else color = "bg-gray-700 text-gray-300 border-gray-500";
            }
